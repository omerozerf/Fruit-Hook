...
namespace _Game._Scripts.MapSystem
{
    public sealed class SpriteMapBaker : MonoBehaviour
    {
        ...
        [SerializeField] private bool _destroySourceObjectsAfterBake = false;
        ...
        public void Bake()
        {
            ...
            for (int i = 0; i < renderers.Count; i++)
            {
                ...
                var tx = sp != null ? sp.texture : null;
            ...
            {
                Debug.LogWarning("[SpriteMapBaker] Bake aborted: detected invalid sprite textures (likely atlas not ready). Retrying next frame.");
                StartCoroutine(BakeAfterDelay());
                return;
            }
        ...
        {
            for (int i = _createdChunks.Count - 1; i >= 0; i--)
            {
                if (_createdChunks[i] != null)
                    Destroy(_createdChunks[i]);
            }

            _createdChunks.Clear();

            for (int i = _outputRoot.childCount - 1; i >= 0; i--)
        ...
        private readonly struct GroupKey : IEquatable<GroupKey>
        {
            ...
            public override int GetHashCode()
            {
                ...
                {
                    int hash = 17;
                    hash = hash * 31 + (Texture != null ? Texture.GetHashCode() : 0);
            ...
            int sortingOrder)
        {
            ...
            for (int cy = minCellY; cy <= maxCellY; cy += chunkSize)
            {
                for (int cx = minCellX; cx <= maxCellX; cx += chunkSize)
                {
        ...
        private static void GetQuadUVFromSpriteRect(Sprite sprite, int uvInsetPixels, out Vector2 bl, out Vector2 tl, out Vector2 tr,
            out Vector2 br)
        {
            var tex = sprite.texture;
        ...
        private void PostBakeCleanup(List<SpriteRenderer> renderers)
        {
            ...
            {
                for (int i = _sourceRoot.childCount - 1; i >= 0; i--)
        ...
        private static List<List<SpriteRenderer>> SplitByVertexBudget(List<SpriteRenderer> renderers, int maxVerticesPerMesh)
        {
