...
namespace LoopGames.Combat
{
	public class SwordOrbitController : MonoBehaviour
	{
		...
		[SerializeField]
		private float _spawnGrowDuration;
		...
		[SerializeField]
		private Transform _swordPrefab;

		[SerializeField]
		private float _spawnInterval = 5f;

		private readonly List<SwordData> m_Swords = new List<SwordData>();
		...
		{
			Transform swordInstance = UnityEngine.Object.Instantiate(_swordPrefab, base.transform);
		...
		{
			m_Swords.RemoveAll((SwordData s) => s.transform == sword);
			RecalculateTargetAngles();
		}
		...
		private void UpdateSwordLocalPositions()
		{
			for (int i = 0; i < m_Swords.Count; i++)
			{
				...
				s.currentAngle = Mathf.SmoothDampAngle(s.currentAngle, s.targetAngle, ref s.angularVelocity, _smoothTime);
		...
		{
			base.transform.Rotate(0f, 0f, (0f - _rotationSpeed) * Time.deltaTime);
		}
		...
		private void RecalculateTargetAngles()
		{
			...
			if (count != 0)
			{
				...
				{
					m_Swords[i].targetAngle = (float)i * angleStep;
				}
			}
		}
		...
		{
			float rad = angle * (3.14159265f / 180f);
